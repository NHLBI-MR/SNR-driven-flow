...
RUN mkdir -p /opt/code/SNR-driven-flow
COPY --chown=$USER_UID:conda SNR-driven-flow/ /opt/code/SNR-driven-flow/
SHELL ["/bin/bash", "-c"]
...
env update --file /opt/code/SNR-driven-flow/environment.yml
