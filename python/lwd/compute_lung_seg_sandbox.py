...
#         print(data.shape)
     ...
     if torch.cuda.is_available():
        ...
        FreeM=0
        device_chosen=0
        for device_i in range(deviceCount):
            handle = nvmlDeviceGetHandleByIndex(device_i)
            info = nvmlDeviceGetMemoryInfo(handle)
            if info.free > FreeM:
                device_chosen=device_i
                FreeM=info.free
        print('Device chosen {} with FreeM {}'.format(device_chosen,FreeM))
        device = torch.device('cuda:{}'.format(device_chosen))
     else:
