...
def correctTrajectoryFluctuations(data_array,navangles):
    ...
    idx = np.argsort(-1*na)
    interleaves = len(una)
    # To discuss with Ahsan  padding data 
    zeropad=data_array.shape[1]% int(interleaves)
    
    if zeropad !=0:
        sorted_signal = np.pad(data_array[:,idx], ((0,0),(0,interleaves-zeropad)), mode='constant', constant_values=0)
    else:
        sorted_signal = data_array[:,idx]

    sorted_signal = cp.reshape(sorted_signal,(sorted_signal.shape[0],int(sorted_signal.shape[1]/interleaves),interleaves))
    ...
    tmp_filtered_signal = butter_highpass_filter(sorted_signal.squeeze(), 0.01, abs(1/(nav_samplingTime)), order=5)
    ...
    filtered_signalX = cp.reshape(tmp_filtered_signal,(sorted_signal.shape[0],sorted_signal.shape[1]*sorted_signal.shape[2]))
    if zeropad !=0:
        filtered_signalX=filtered_signalX[:,0:data_array.shape[1]]
    filtered_signal= np.zeros(data_array.shape)
...
def BinningGadget(connection):
    ...
    if "binningPercent" in params:
        binningPercent = int(params["binningPercent"])
    else:    
        binningPercent = 40
    ...
    acceptedTimes = binning(respiratory_waveform,nav_tstamp,binningPercent,bidirectional, do_stable_binning, evenbins, numBins )
