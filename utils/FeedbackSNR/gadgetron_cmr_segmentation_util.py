...
def export_prediction_from_softmax_onnx(predicted_array: np.ndarray,info_preprocess: dict, properties_dict: dict,output_file_name:str=None):
    ...
    #Resampling
    predicted_array = resample_data_or_seg_to_shape(predicted_array,properties_dict['shape_after_cropping_and_before_resampling'],current_spacing,properties_dict['spacing'])

    #segmentation does not handle multiple regions  mix(features nnUnet hasregion) 
    predicted_probabilities=np_sigmoid(predicted_array)
    ...
    # revert transpose
    segmentation_reverted_cropping = segmentation_reverted_cropping.transpose(info_preprocess['transpose_backward'])
